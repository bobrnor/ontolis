...
class OntToJavaConverter:
    ...
    def _bypass_methods(self, methods):
        for method in methods:
            ...
            self._code += "public " + self._get_type_name(method_type[0].name) + " "
    ...
    def _bypass_method_parameters(self, parameters):
        ...
        for parameter in parameters:
            ...
            if "#" not in parameter.name:
                match = re.match(objc_parameter_parse, parameter.name)
                type = match.group(2)
                name = match.group(3)
                if not is_first:
                    self._code += ", "
                    ...
                    self._code += method_name + "("
                self._code += type + " " + name
                is_first = False
        self._code += ") "
