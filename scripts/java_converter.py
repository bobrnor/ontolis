...
class JavaToOntConverter:
    def __init__(self, java_file):
        self._file = java_file
        self._current_class = None
        self._current_method = None
        self._ontology = Ontology()
        self._ontology.from_json(json.load(file(jsbn_path)))
        self._ontology.create_node_if_needed("##.java")
    ...
    def _process_line(self, line):
        if re.match(java_simple_class_regexp, line):
            self._process_simple_class(line)
        elif re.match(java_extending_class_regexp, line):
            self._process_extending_class(line)
        elif re.match(java_implementing_class_regexp, line):
            self._process_implementing_class(line)
        elif re.match(java_method_regexp, line):
            self._process_method(line)

    def _process_simple_class(self, line):
        match = re.match(java_simple_class_regexp, line)
        desc = {}
        modifiers = match.group(1)
        if modifiers:
            desc["modifiers"] = modifiers.split()
        desc["name"] = match.group(2)
        # print desc
        self._current_class = self._ontology.add_class(desc)

    def _process_extending_class(self, line):
        match = re.match(java_extending_class_regexp, line)
        desc = {}
        modifiers = match.group(1)
        if modifiers:
            desc["modifiers"] = modifiers.split()
        desc["name"] = match.group(2)
        desc["super"] = match.group(3)
        # print desc
        self._current_class = self._ontology.add_class(desc)

    def _process_implementing_class(self, line):
        match = re.match(java_implementing_class_regexp, line)
        desc = {}
        modifiers = match.group(1)
        if modifiers:
            desc["modifiers"] = modifiers.split()
        desc["name"] = match.group(2)
        desc["interfaces"] = match.group(3).split(",")
        # print desc
        self._current_class = self._ontology.add_class(desc)

    def _process_method(self, line):
        match = re.match(java_method_regexp, line)
        desc = {}
        desc["modifiers"] = match.group(1).split()
        desc["name"] = match.group(3)
        desc["parameters"] = match.group(4).split(",")
        # print desc
...
class OntToJavaConverter:
    ...
    def _bypass_by_classes(self, class_nodes):
        for class_node in class_nodes:
            ...
            # print super_class[0]
            if super_class[0]:
                self._code += "extends " + super_class[0].name + " "
...
def convert_to_internal_format(file_path, ontology_path):
    converter = JavaToOntConverter(open(file_path))
    print converter.convert()


def convert_to_external_format(file_path):
    converter = OntToJavaConverter(open(file_path))
    print converter.convert()
